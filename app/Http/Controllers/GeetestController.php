...
class GeetestController extends Controller
{
    ...
    public function first_register(Request $request)
    {
        ...
        $request->session()->put(GeetestLib::GEETEST_SERVER_STATUS_SESSION_KEY, $result->getStatus());
    ...
    public function second_validate(Request $request)
    {
        ...
        $status = $request->session()->get(GeetestLib::GEETEST_SERVER_STATUS_SESSION_KEY, null);
        $userId = $request->session()->get("userId", null);
        if ($status === null) {
            return response()->json(["result" => "fail", "version" => GeetestLib::VERSION, "msg" => "session取key发生异常"]);
        }
        $result = null;
        if ($status === 1) {
            /*
